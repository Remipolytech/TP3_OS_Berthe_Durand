...
// pour que ça compile sur Mac (macOS ne connaît pas MSG_CONFIRM)
#ifndef MSG_CONFIRM
...
socklen_t ls;
    ...
    for (;;) {
      ...
      if ((n=recvfrom(sid,(void*)buf,LBUF,0,(struct sockaddr *)&Sock,&ls))
        ...
        char *ar_msg = "Bien reçu 5/5 !";
        if (sendto(sid, ar_msg, strlen(ar_msg), MSG_CONFIRM, (struct sockaddr *)&Sock, ls) == -1) {
            perror("sendto AR");
        }
      }
    }
